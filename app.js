...
function actualizarEstadoConexion(){
    ...
    let pendientes=0;
    if(dbOffline){
        Promise.all([dbAll("operaciones"),dbAll("operacionesXCC")]).then(a=>{
            pendientes=a[0].length+a[1].length;
        ...
        indicador.className=navigator.onLine?"":"conexion-offline";
        indicador.innerHTML=`<span class="conexion-punto"></span>${navigator.onLine?"Conectado":"Sin conexión"}`;
    }
}
async function sincronizarPendientes(){
    if(!navigator.onLine||!dbOffline||sincronizando)return;
    sincronizando=true;actualizarEstadoConexion();
    try{
        const ops=await dbAll("operaciones");
        for(const op of ops){
            try{
                const respuesta=await fetch(API_URL,{method:"POST",headers:{"Content-Type":"text/plain;charset=utf-8"},body:JSON.stringify(op.payload)});
                ...
                }else if(op.payload.accion==="editarMovimiento"){
                    const item=registros.find(r=>r.id===op.payload.localId);
                    if(item){item.sincronizado=true;item.fila=resultado.fila||item.fila;}
                    if(resultado.fila)await dbPut("movimientos",item);
                }else{
                    const item=registros.find(r=>r.id===op.payload.localId);
    ...
    }finally{sincronizando=false;actualizarEstadoConexion();actualizarMovimientos();actualizarResumen();}
}
window.addEventListener("online",async()=>{actualizarEstadoConexion();await sincronizarPendientes();await sincronizarXCCPendientes();actualizarEstadoConexion();});
...
async function cargarCatalogos(){
    try{
        ...
        if(navigator.onLine){
            try{
                const respuesta=await fetch(API_URL+"?accion=obtenerCatalogos",{cache:"no-store"});
                const resultado=await respuesta.json();
            ...
            if(!cache)throw new Error("No hay catálogos guardados localmente. Conéctate una vez para descargarlos.");
            clientes=cache.clientes||[];
            elementos=cache.elementos||[];
        }
...
async function cargarInventario(){
    try{
        ...
        if(localesHoy.length){registros=localesHoy.sort((a,b)=>(a.hora||"").localeCompare(b.hora||""));actualizarResumen();actualizarMovimientos();}
        if(navigator.onLine){
            const respuesta=await fetch(API_URL+"?accion=obtenerInventario&fecha="+encodeURIComponent(fechaTexto),{cache:"no-store"});
            const resultado=await respuesta.json();
            if(!resultado.ok)throw new Error(resultado.error);
            const servidor=resultado.registros||[];
            const pendientes=registros.filter(r=>r.sincronizado===false);
            const fusion=[...servidor];
            pendientes.forEach(p=>{if(!fusion.some(s=>(s.fila&&p.fila&&s.fila===p.fila)||s.id===p.id))fusion.push(p);});
            registros=fusion;
            for(const r of servidor){r.sincronizado=true;await dbPut("movimientos",r);}
            actualizarResumen();actualizarMovimientos();
            await sincronizarPendientes();
        }
    }catch(error){console.error(error);actualizarResumen();actualizarMovimientos();if(navigator.onLine)mostrarMensaje("❌ No se pudo cargar el inventario","error");}
}
...
async function registrarInventario(){
    ...
    if(!dbOffline)await abrirDBOffline();
    const localId=generarIdLocal("mov");
    ...
    await dbPut("movimientos",datos);
    await dbPut("operaciones",{id:generarIdLocal("op"),payload:{...datos,localId}});
    limpiarFormulario();actualizarResumen();actualizarMovimientos();actualizarEstadoConexion();
    mostrarMensaje(navigator.onLine?"✓ Guardado localmente. Sincronizando...":"✓ Guardado sin conexión. Se sincronizará al volver internet","ok");
...
function prepararDatosReporte(){
    ...
    registros.forEach(registro=>{
        // El reporte de INVENTARIO DIARIO solo debe incluir
        // movimientos que tengan cliente, elemento, cantidad
        // y un estado válido. Esto evita que datos de XCC u
        // otros registros auxiliares terminen en este reporte.
        const cliente=String(registro?.cliente||"").trim();
        const elementoTexto=String(registro?.elemento||"").trim();
        const cantidad=Number(registro?.cantidad)||0;
        const estado=normalizarTexto(registro?.estado||"");

        if(!cliente || !elementoTexto || cantidad<=0) return;
        if(estado!=="MONO" && estado!=="PROCESO") return;

        if(!grupos[cliente])grupos[cliente]=[];

        let elemento=grupos[cliente].find(
            item=>normalizarTexto(item.elemento)===normalizarTexto(elementoTexto)
        );
...
async function iniciarAplicacion(){
    try{await abrirDBOffline();actualizarEstadoConexion();await cargarCatalogos();await cargarInventario();await cargarDatosXCC();actualizarEstadoConexion();if(navigator.onLine)sincronizarPendientes();}catch(error){console.error(error);actualizarEstadoConexion();}
}
...
async function guardarXCCLocal(){
    ...
    for(const f of datosXCC){
        ...
        await dbPut("xcc",r);
        await dbPut("operacionesXCC",{id:generarIdLocal("opxcc"),payload:{...r,accion:"registrarInventarioXCC"}});
    }
    renderizarXCC();
    actualizarEstadoConexion();
    mensajeXCC(navigator.onLine?"✓ Guardado local. Sincronizando...":"✓ Guardado sin conexión. Quedó pendiente de sincronizar.","ok");
    if(navigator.onLine)await sincronizarXCCPendientes();
    actualizarEstadoConexion();
}
async function sincronizarXCCPendientes(){
    ...
    for(const op of ops){
        try{
            const r=await fetch(API_URL,{method:"POST",headers:{"Content-Type":"text/plain;charset=utf-8"},body:JSON.stringify(op.payload)});
...
function generarReporteXCC(){
    ...
    const ft=fecha.toLocaleDateString("es-CO",{day:"2-digit",month:"2-digit",year:"numeric"});
    const ht=fecha.toLocaleTimeString("es-CO",{hour:"2-digit",minute:"2-digit",second:"2-digit",hour12:false});
    ...
    const canvas=document.createElement("canvas");
    const w=1200;
    const filas=Math.max(datosXCC.length,1);
    const h=340+(filas*58)+14+150;
    canvas.width=w;
    canvas.height=h;
    ...
    c.beginPath();
    c.moveTo(calX+7,calY-7); c.lineTo(calX+7,calY+4);
    c.moveTo(calX+27,calY-7); c.lineTo(calX+27,calY+4);
    c.stroke();
    c.lineWidth=2;
    c.beginPath();
    for(let i=0;i<3;i++){
    ...
    c.fillText(ft.replace(/\//g,"-"),105,65);
    c.font="bold 18px Arial";
    c.fillText("Fecha del inventario",106,91);

    // TITULO CENTRAL — IGUAL AL INVENTARIO DIARIO
    ...
    if(logoServitodo.complete&&logoServitodo.naturalWidth>0){
        ...
        logoW*=escala;
        logoH*=escala;
        c.drawImage(logoServitodo,areaX+(areaW-logoW)/2,areaY+(areaH-logoH)/2,logoW,logoH);
    }

    // =================================================
    // TABLA XCC — CONSERVA TODOS LOS DATOS ACTUALES
    // CON EL MISMO LENGUAJE VISUAL DEL INVENTARIO DIARIO
    // =================================================
    const tablaX=14;
    const tablaY=146;
    const tablaW=w-(tablaX*2);
    const encabezadoH=76;

    const columnas=[260,150,185,160,170,247];
    const nombres=["CLIENTE","ESTIBAS/DÍA","INVENTARIO ACTUAL","DÍAS CUBIERTOS","OBJETIVO","FALTANTE"];
    const xs=[tablaX];
    for(let i=0;i<columnas.length-1;i++)xs.push(xs[i]+columnas[i]);

    c.fillStyle=grisClaro;
    c.fillRect(tablaX,tablaY,tablaW,encabezadoH);
    c.strokeStyle=borde;
    c.lineWidth=1;
    c.strokeRect(tablaX,tablaY,tablaW,encabezadoH);

    c.fillStyle=azulTexto;
    c.font="bold 17px Arial";
    c.textAlign="center";
    nombres.forEach((nombre,i)=>{
        const x=xs[i]+columnas[i]/2;
        if(nombre==="INVENTARIO ACTUAL"){
            c.fillText("INVENTARIO",x,tablaY+31);
            c.fillText("ACTUAL",x,tablaY+53);
        }else if(nombre==="DÍAS CUBIERTOS"){
            c.fillText("DÍAS",x,tablaY+31);
            c.fillText("CUBIERTOS",x,tablaY+53);
        }else if(nombre==="ESTIBAS/DÍA"){
            c.fillText("ESTIBAS/DÍA",x,tablaY+42);
        }else{
            c.fillText(nombre,x,tablaY+42);
        }
    });

    for(let i=1;i<columnas.length;i++){
        c.beginPath();
        c.moveTo(xs[i],tablaY);
        c.lineTo(xs[i],tablaY+encabezadoH+(filas*58));
        c.stroke();
    }

    let y=tablaY+encabezadoH;
    ...
    datosXCC.forEach((f,i)=>{
        ...
        c.fillRect(tablaX,y,tablaW,58);
        c.strokeStyle=borde;
        c.beginPath();
        c.moveTo(tablaX,y+58);
        c.lineTo(tablaX+tablaW,y+58);
        c.stroke();

        c.textAlign="left";
        c.fillStyle=azulTexto;
        c.font="bold 18px Arial";
        textoAjustado(c,f.cliente,xs[0]+18,y+36,columnas[0]-36);

        c.textAlign="center";
        c.font="bold 18px Arial";
        c.fillStyle=azulTexto;
        c.fillText(formatear(f.estibasDia),xs[1]+columnas[1]/2,y+36);
        c.fillText(formatear(f.inventarioActual),xs[2]+columnas[2]/2,y+36);
        c.fillText(dias.toFixed(1),xs[3]+columnas[3]/2,y+36);
        c.fillText(formatear(f.objetivo),xs[4]+columnas[4]/2,y+36);

        c.fillStyle=faltante>0?"#c62828":faltante<0?"#168548":grisTexto;
        c.fillText(formatear(faltante),xs[5]+columnas[5]/2,y+36);
    ...
    c.strokeStyle=borde;
    c.lineWidth=2;
    c.strokeRect(tablaX,tablaY,tablaW,encabezadoH+(datosXCC.length*58));

    // =================================================
    // PIE — MISMO DISEÑO DEL INVENTARIO DIARIO
    // =================================================
    const footerY=tablaY+encabezadoH+(datosXCC.length*58)+24;
    const footerH=116;
    const f1=tablaX;
    const f2=tablaX+tablaW/3;
    const f3=tablaX+(tablaW/3)*2;
    ...
    c.beginPath();
    c.moveTo(f2,footerY); c.lineTo(f2,footerY+footerH);
    c.moveTo(f3,footerY); c.lineTo(f3,footerY+footerH);
    c.stroke();
    ...
    c.stroke();
    c.beginPath();
    c.moveTo(f1+40,footerY+44); c.lineTo(f1+40,footerY+31);
    c.moveTo(f1+40,footerY+44); c.lineTo(f1+49,footerY+49);
    c.stroke();
    ...
    c.fillStyle=azulTexto;
    c.font="bold 18px Arial";
    c.fillText(ft+" "+ht,f1+73,footerY+62);
    ...
    c.beginPath();
    c.moveTo(docX-6,footerY+34); c.lineTo(docX+6,footerY+34);
    c.moveTo(docX-6,footerY+41); c.lineTo(docX+6,footerY+41);
    c.moveTo(docX-6,footerY+48); c.lineTo(docX+6,footerY+48);
    c.stroke();
    c.fillStyle=grisTexto;
    c.font="15px Arial";
    c.fillText("Inventario diario Servitodo",f3+73,footerY+32);
    c.fillStyle=azulTexto;
    c.font="bold 18px Arial";
